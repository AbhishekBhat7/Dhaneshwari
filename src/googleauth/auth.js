...
const authregister = async (req, res) => {
  const { email, name, firebase_id } = req.body;  // changed firebase_uid -> firebase_id

  if (!email || !name || !firebase_id) {
    return res.status(400).json({ message: 'Missing required fields' });
  }
  ...
  try {
    const checkQuery = `
      ...
      WHERE email = $1 OR firebase_id = $2
    `;
    const checkValues = [email, firebase_id];
    const checkResult = await pool.query(checkQuery, checkValues);

    if (checkResult.rows.length > 0) {
      return res.status(409).json({ message: 'User already exists with this email or firebase_id' });
    }

    const query = `
      INSERT INTO client (email, name, firebase_id)
      VALUES ($1, $2, $3)
      RETURNING email
    `;
    const values = [email, name, firebase_id];
